...
export function printInvoice(invoice: Invoice, job: Job, userProfile: UserProfile): void {
  ...
  const businessName = escapeHtml(userProfile.businessName.toUpperCase())
  const invoiceNumber = escapeHtml(invoice.number)
  const paymentStatus = escapeHtml(invoice.paymentStatus.toUpperCase())
  const paymentTerms = escapeHtml(invoice.paymentTerms)
  const profileName = escapeHtml(userProfile.name)
  const profileAddress = escapeHtml(userProfile.address ?? '')
  const profilePhone = escapeHtml(userProfile.phone ?? '')
  const clientName = escapeHtml(job.clientName)
  const jobAddress = escapeHtml(job.address)
  const notes = invoice.notes ? escapeHtml(invoice.notes) : ''
...
<div class="title-row">
  <div><h2>INVOICE</h2><div class="inv-num">#${invoiceNumber} · ${new Date(invoice.createdAt).toLocaleDateString()}</div></div>
  <div style="text-align:right;font-size:13px">
    <div style="font-weight:700;color:#FF6B2B">${paymentStatus}</div>
    <div style="color:#999">${paymentTerms}</div>
  </div>
</div>
<div style="display:flex;gap:40px;margin-bottom:28px">
  <div><div style="font-size:11px;color:#999;font-weight:700;letter-spacing:1px;margin-bottom:4px">FROM</div>
    <strong>${profileName}</strong><br>${profileAddress}<br>${profilePhone}</div>
  <div><div style="font-size:11px;color:#999;font-weight:700;letter-spacing:1px;margin-bottom:4px">BILL TO</div>
    <strong>${clientName}</strong><br>${jobAddress}</div>
</div>
<table>
...
<tbody>
${invoice.lineItems.map((li) => `<tr><td>${escapeHtml(li.description)}</td><td style="text-align:right">${li.quantity}</td><td style="text-align:right">$${li.unitPrice.toFixed(2)}</td><td style="text-align:right;font-weight:600">$${li.total.toFixed(2)}</td></tr>`).join('')}
</tbody>
</table>
...
${notes ? `<div style="clear:both;margin-top:28px;padding:12px 16px;background:#F5F0EB;border-left:3px solid #FF6B2B;font-size:13px">${notes}</div>` : ''}

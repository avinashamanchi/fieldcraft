...
export const createHandler =
  ...
  async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      let raw: unknown;
      try {
        raw = JSON.parse(await readBoundedBody(request, MAX_REQUEST_BYTES));
      } catch (cause) {
      ...
      if (!userRate.allowed || !networkRate.allowed) {
        ...
        return json(
          status,
          { error: publicCode, requestId, retryAfterSeconds },
      ...
      } else if (code === "body-too-large") {
        status = 413;
        publicCode = "request-too-large";
      } else if (code === "invalid-request") {
        status = 400;
        publicCode = "invalid-request";
      } else if (code === "provider-timeout") {
        status = 504;
        publicCode = "upstream-timeout";
      } else if (code === "invalid-provider-response") {
        status = 502;
        publicCode = "invalid-upstream-response";
      } else {
        status = 503;
      ...
      dependencies.log({
        requestId,
        route: route ?? "unparsed",
        userDigest: userDigest ?? "unavailable",
        networkDigest: networkDigest ?? "unavailable",
        status,
        publicCode,
...
export const createProductionHandler = (
  ...
  const hmacSecret = requireEnvironment(
    environment,
    "AI_RATE_LIMIT_HMAC_SECRET",
  );
  const publishableKey = environment.SUPABASE_ANON_KEY?.trim() ||
    requireEnvironment(environment, "SUPABASE_PUBLISHABLE_KEY");
  ...
  return createHandler({
    allowedOrigins: new Set(
      (environment.AI_ALLOWED_ORIGINS ?? "https://avinashamanchi.github.io")
        .split(",").map((value) => value.trim()).filter(Boolean),
    ),
    authenticateRequest: (authorization) =>
      authenticate(authorization, { supabaseUrl, publishableKey }),
    consume: (scope, route, limit) =>
      consumeRateLimit(scope, route, limit, { supabaseUrl, serviceRoleKey }),
    digest: (value) => digestScope(hmacSecret, value),
    invokeProvider: (request) =>
      callProvider(request, {
        apiKey: providerKey,
        endpoint: environment.AI_PROVIDER_URL ??

...
const make = (overrides: Record<string, unknown> = {}) => {
  ...
  const handler = createHandler({
    ...
    digest: (value) => Promise.resolve(`digest-${value.split(":")[0]}`),
    invokeProvider: () => Promise.resolve({ category: "Materials" }),
...
Deno.test("rejects non-POST, missing auth, wrong route, unknown fields, and oversized input", async () => {
  ...
  assert(
    (await handler(
      new Request("https://edge.example", {
        method: "POST",
        body: JSON.stringify({ ...valid, route: "wrong" }),
      }),
    )).status === 400,
    "route",
  );
...
Deno.test("rejects wrong consent and a 20001-code-point transcript", async () => {
  const { handler } = make();
  const wrongConsent = await handler(
    new Request("https://edge.example", {
      method: "POST",
      body: JSON.stringify({ ...valid, consentVersion: "old" }),
    }),
  );
...
Deno.test("applies independent rate limits and returns an integer retry", async () => {
  const scopes: string[] = [];
  const { handler } = make({
    consume: (scope: string) => {
      scopes.push(scope);
      return Promise.resolve({
        allowed: scope === "digest-user",
  ...
  assert(
    response.status === 429 && body.retryAfterSeconds === 17,
    "rate result",
  );
  assert(
    scopes.includes("digest-user") && scopes.includes("digest-network"),
    "independent scopes",
  );
});
...
Deno.test("returns versioned output without logging or exposing private content", async () => {
  ...
  const { handler, logs } = make({
    invokeProvider: (_request: AiRequest) =>
      Promise.resolve({ category: "Other" }),
  });
  const response = await handler(
    new Request("https://edge.example", {
      method: "POST",
      body: JSON.stringify({ ...valid, notes: marker }),
    }),
  );
  const text = await response.text();
  assert(
    response.status === 200 && text.includes('"version":1'),
    "versioned output",
  );
...
Deno.test("maps provider timeout and invalid provider response to content-free public errors", async () => {
  ...
  for (
    const [message, status] of [["provider-timeout", 504], [
      "invalid-provider-response",
      502,
    ]] as const
  ) {
    const { handler } = make({
      invokeProvider: () => Promise.reject(new Error(message)),
    });
    const response = await handler(
      new Request("https://edge.example", {
        method: "POST",
...
Deno.test("provider access enforces its deadline, byte cap, and strict schema", async () => {
  ...
  const oversized = JSON.stringify({
    choices: [{ message: { content: "x".repeat(129 * 1024) } }],
  });
  await assertRejects(
    callProvider(valid as AiRequest, {
      ...base,
      fetcher: () => Promise.resolve(new Response(oversized)),
    }),
    "invalid-provider-response",
  );
  const malformed = JSON.stringify({
    choices: [{ message: { content: '{"category":"Not allowed"}' } }],
  });
  await assertRejects(
    callProvider(valid as AiRequest, {
      ...base,
      fetcher: () => Promise.resolve(new Response(malformed)),
    }),
    "invalid-provider-response",
  );
});
